...
bl_sr_RT_result_AIC <- vector(
    mode = "numeric",
    length = length(subject_list)
)
bl_sr_RT_decay_result_AIC <- vector(
    mode = "numeric",
    length = length(subject_list)
)
...
for (i in seq_len(length(subject_list))) {
    ...
    bl_ab_RT_data <- select(tmp_data, RT, bl_ab_RT, starts_with("mini_block"))
    model_bl_ab_RT <- lm(RT ~ . , data = bl_ab_RT_data)

    bl_sr_data <- select(tmp_data, RT, bl_sr, starts_with("mini_block"))
    model_bl_sr <- lm(RT ~ ., data = bl_sr_data)

    bl_sr_RT_data <- select(tmp_data, RT, bl_sr_RT, starts_with("mini_block"))
    model_bl_sr_RT <- lm(RT ~ ., data = bl_sr_RT_data)

    bl_sr_RT_decay_data <- select(tmp_data, RT, bl_sr_RT_decay, starts_with("mini_block"))
    model_bl_sr_RT_decay <- lm(RT ~ ., data = bl_sr_RT_decay_data)
    ...
    rl_sr_decay_data <- select(
        tmp_data, RT, rl_sr_decay,
        starts_with("mini_block")
    )
    model_rl_sr_decay <- lm(RT ~ ., data = rl_sr_decay_data)

    bl_ab_result_AIC[i] <- AIC(model_bl_ab)
    bl_ab_RT_result_AIC[i] <- AIC(model_bl_ab_RT)
    bl_sr_result_AIC[i] <- AIC(model_bl_sr)
    bl_sr_RT_result_AIC[i] <- AIC(model_bl_sr_RT)
    bl_sr_RT_decay_result_AIC[i] <- AIC(model_bl_sr_RT_decay)
...
AIC_result_table <- data.frame(
    ...
    bl_ab_RT = bl_ab_RT_result_AIC,
    bl_sr = bl_sr_result_AIC,
    bl_sr_RT = bl_sr_RT_result_AIC,
    bl_sr_RT_decay = bl_sr_RT_decay_result_AIC,
