...
static PyObject *SR_Learning_without_beta(PyObject *self, PyObject *args)
{
	...
	if (!strcmp(model_type, "SR_Q_WOB") && para_len == 1) //Q-learning: alpha
	{
		alpha = PyFloat_AsDouble(PyList_GetItem(parameters, 0));
	}
	else if (!strcmp(model_type, "SR_Q_D_WOB") && para_len == 2) //Q-learning with forgetting: alpha, decay
	{
		alpha = PyFloat_AsDouble(PyList_GetItem(parameters, 0));
		decay = PyFloat_AsDouble(PyList_GetItem(parameters, 1));
	}
	else if (!strcmp(model_type, "SR_Q_D_E_WOB") && para_len == 3) //Q-learning with forgetting and error: alpha, decay, alpha_error
	{
		alpha = PyFloat_AsDouble(PyList_GetItem(parameters, 0));
		decay = PyFloat_AsDouble(PyList_GetItem(parameters, 1));
		alpha_error = PyFloat_AsDouble(PyList_GetItem(parameters, 2));
	}
	else if (!strcmp(model_type, "SR_Q_D_alphaCCC_WOB") && para_len == 4) //Q-learning with forgetting and cognitive control:alpha, decay, ccc, alpha_ccc
	{
	...
	for (i = 0; i < trial_num; i++)
	{
		...
		else if (!strcmp(model_type, "SR_Q_D_alphaCCC_WOB") && para_len == 4)
		{
			conflict = 2 * P - 1;
