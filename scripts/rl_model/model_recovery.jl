...
for i in sub_num_list
    ...
    model_result = ab_volatility_model(ab_v_params.α_s[1], ab_v_params.α_v[1],
                                       stim_feature_seq, exp_volatility_seq)
    ...
    model_result = sr_sep_alpha_model(sr_params.α_l[1], sr_params.α_r[1], stim_loc_seq,
                                      reaction_loc_seq)
    sr_p_value = [sr_p_value; model_result["Predicted sequence"]]
    sr_pe_value = [sr_pe_value; model_result["Prediciton error"]]
    sr_ll = [sr_ll; model_result["Predicied Left sequence"]]
    sr_rl = [sr_rl; model_result["Predicied Right sequence"]]

    # sr_v model
    sr_v_params = @subset(sr_v_optim_params, :sub .== i)
    model_result = sr_sep_alpha_volatility_model(sr_v_params.α_s_l[1], sr_v_params.α_s_r[1],
                                                 sr_v_params.α_v_l[1], sr_v_params.α_v_r[1],
                                                 stim_loc_seq, reaction_loc_seq,
                                                 exp_volatility_seq)
    sr_v_p_value = [sr_v_p_value; model_result["Predicted sequence"]]
    sr_v_pe_value = [sr_v_pe_value; model_result["Prediciton error"]]
    sr_v_ll = [sr_v_ll; model_result["Predicied Left sequence"]]
    sr_v_rl = [sr_v_rl; model_result["Predicied Right sequence"]]
end

insertcols!(all_sub_data, :rl_ab_p => ab_p_value, :rl_ab_pe => ab_pe_value,
            :rl_ab_v_p => ab_v_p_value, :rl_ab_v_pe => ab_v_pe_value,
            :rl_sr_p => sr_p_value, :rl_sr_pe => sr_pe_value, :rl_sr_v_p => sr_v_p_value,
            :rl_sr_v_pe => sr_v_pe_value, 
            :rl_sr_ll => sr_ll, :rl_sr_rl => sr_rl,
            :rl_sr_v_ll => sr_v_ll, :rl_sr_v_rl => sr_v_rl)
CSV.write("/Users/dddd1007/project2git/cognitive_control_model/data/input/all_data_with_rl_model_estimate_by_stim.csv",
          all_sub_data)
