...
for (sub_num in unique(sub_data$Subject_num)) {
    ...
    sub_data_list[[sub_num]] <- cbind(single_sub_data,
                                      ...
                                      SR_Decay_ll = tmp_result_list[[2]]$Q_ll,
                                      SR_Decay_lr = tmp_result_list[[2]]$Q_lr,
                                      SR_Decay_rl = tmp_result_list[[2]]$Q_rl,
                                      SR_Decay_rr = tmp_result_list[[2]]$Q_rr,
...
for (sub_num in unique(sub_data$Subject_num)) {
    ...
    # 将单个被试的数据与模型结果合并
    scaled_sub_data_list[[sub_num]] <- cbind(single_sub_data,
                                      AB = range01(tmp_result_list[[1]]$P),
                                      SR_Decay = range01(tmp_result_list[[2]]$P),
                                      SR_Decay_ll = range01(tmp_result_list[[2]]$Q_ll),
                                      SR_Decay_lr = range01(tmp_result_list[[2]]$Q_lr),
                                      SR_Decay_rl = range01(tmp_result_list[[2]]$Q_rl),
                                      SR_Decay_rr = range01(tmp_result_list[[2]]$Q_rr),
                                      SR = range01(tmp_result_list[[3]]$P))
    #####
}
all_sub_data_scaled <- bind_rows(scaled_sub_data_list, .id = "column_label")
write.csv(all_sub_data_scaled, "/Users/dddd1007/project2git/cognitive_control_model/data/input/all_data_with_rl_model_scaled.csv")