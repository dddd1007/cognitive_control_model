...
import CSV, cognitive_control_model
# set the generated file locations
csvpath = joinpath(dirname(pathof(cognitive_control_model)), "..",  "data", "output", "RLModels", "AB")

# import all data
all_data = CSV.read(joinpath(dirname(pathof(cognitive_control_model)), "..", "data", "input", "pure_all_data.csv"));
...
Threads.@threads for sub_num in 1:36
    ...
    optim_param, eval_result, verbose_table = Models.Optim.RL_NoSoftMax_basic_AB(each_env, each_subinfo, 10000)
    ...
    CSV.write(csvpath * "/sub_" * repr(sub_num) * "_params_basic_verbose.csv", verbose_table)
