...
model {
  ...
  for(t in 1:N){
    if(space_loc[t] == 0){
        if(volatility[t] == 0){
            p_l[t + 1] = p_l[t] + alpha_s * (corr_react - p_l[t]);
        }else{
            p_l[t + 1] = p_l[t] + alpha_v * (corr_react - p_l[t]);
        }
        p_r[t + 1] = p_r[t];
        ...
        p_l[t + 1] = p_l[t];
        if(volatility[t] == 0){
            p_r[t + 1] = p_r[t] + alpha_s * (corr_react - p_r[t]);
        }else{
            p_r[t + 1] = p_r[t] + alpha_v * (corr_react - p_r[t]);
        }
        p_r[t + 1] = p_r[t];
