...
class RLCCModels(object):
    def __init__(self, model_type, data):
        ...
        self.PResult, self.QResult, self.PEResult, self.conflict = [],[],[],[]
        self.Q_name = []
        self.PE_name = []
        self.model_para = []
        ...
        if model_type == 'SR_Q_WOB':
            ...
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['relevant', 'alpha']
            self.model_para_bounds = [self.relevant_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_WOB':
            # alpha, decay
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['relevant', 'alpha', 'decay']
            self.model_para_bounds = [self.relevant_bound, self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_E_WOB':
            # alpha, decay, alpha_error
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['relevant', 'alpha', 'decay', 'alpha_error']
            self.model_para_bounds = [self.relevant_bound, self.alpha_bound, self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_alphaCCC_WOB':
            # alpha, decay, ccc, alpha_ccc
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['relevant', 'alpha', 'decay', 'ccc', 'alpha_ccc']
            self.model_para_bounds = [self.relevant_bound, self.alpha_bound, self.alpha_bound, self.ccc_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_E_alphaCCC_WOB':
            # alpha, decay, alpha_error, ccc, alpha_ccc
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['relevant', 'alpha', 'decay', 'alpha_error', 'ccc', 'alpha_ccc']
            self.model_para_bounds = [self.relevant_bound, self.alpha_bound, self.alpha_bound, self.alpha_bound, self.ccc_bound,
                                      self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
    ...
    def SR_Learning(self, parameters):
        ...
        if model_type[-3:] == 'WOB':
            P, Q, pe, conflict = RLCC.SR_Learning_without_beta(model_type, parameters.tolist(), stimPosit, respPosit, correction)
        else:
    ...
    def save_data(self):
        ...
        conflict = pd.DataFrame(self.conflict)
        conflict.columns = ['conflict']
        modelVars = pd.concat([PResult, QResult, PEResult, conflict], axis=1)
