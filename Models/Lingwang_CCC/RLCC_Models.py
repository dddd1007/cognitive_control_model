...
if sys.platform == 'linux':
    ...
    #sys.path.append('./RLCC/x64/Release')
import RLCC
...
class RLCCModels(object):
    def __init__(self, model_type, data):
        ...
        if model_type == 'SR_Q_WOB':
            ...
            self.model_para_bounds = [self.alpha_bound, self.alpha_bound, self.alpha_bound, self.ccc_bound,
                                      self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_V_WOB':
            # alpha
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = [ 'alpha', 'alpha_v']
            self.model_para_bounds = [self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_V_WOB':
            # alpha, decay
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = [ 'alpha', 'decay', 'alpha_v']
            self.model_para_bounds = [self.alpha_bound, self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_E_V_WOB':
            # alpha, decay, alpha_error
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['alpha', 'decay', 'alpha_error', 'alpha_v']
            self.model_para_bounds = [self.alpha_bound, self.alpha_bound, self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_alphaCCC_V_WOB':
            # alpha, decay, ccc, alpha_ccc
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = ['alpha', 'decay', 'ccc', 'alpha_ccc', 'alpha_v']
            self.model_para_bounds = [self.alpha_bound, self.alpha_bound, self.ccc_bound, self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
        elif model_type == 'SR_Q_D_E_alphaCCC_V_WOB':
            # alpha, decay, alpha_error, ccc, alpha_ccc
            self.Q_name = ['Q_ll', 'Q_lr', 'Q_rl', 'Q_rr']
            self.PE_name = ['PE_ll', 'PE_lr', 'PE_rl', 'PE_rr']
            self.model_para_names = [ 'alpha', 'decay', 'alpha_error', 'ccc', 'alpha_ccc', 'alpha_v']
            self.model_para_bounds = [self.alpha_bound, self.alpha_bound, self.alpha_bound, self.ccc_bound,
                                      self.alpha_bound, self.alpha_bound]
            self.parameter_size = len(self.model_para_names)
    ...
    def SR_Learning(self, parameters):
        ...
        if model_type[-5:] == 'V_WOB':
            volatility = self.data['condition'].tolist()
            P, Q, pe = RLCC.SR_Learning_without_beta_volatility(model_type, parameters.tolist(), stimPosit, respPosit, correction, volatility)
        elif model_type[-3:] == 'WOB':
